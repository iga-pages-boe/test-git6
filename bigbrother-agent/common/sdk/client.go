...
import (
	...
	"code.byted.org/ti/bigbrother-agent/common/watcher"
	...
	for {
		...
		case <-ticker:
			...
			for k, v := range versionMap {
				mmetrics.EmitStore(1, metrics.T{"versionKey", k}, metrics.T{"version", v})
			}
			for k, v := range letancyMap {
				mmetrics.EmitStore(1, metrics.T{"latencyKey", k}, metrics.T{"latency", v})
			}
...
func (c *Client) AddLocalKeyListener(key string, cb Callback) error {
	md := metadata.Pairs("from_agent_cli", "true")
	ctx := metadata.NewOutgoingContext(context.Background(), md)
	_, err := c.Get(ctx, key)
	if err != nil {
		if err == errcode.NotFound {
			logs.Debug("not found local key: %s", key)
		} else {
			return err
		}
	}
	return c.AddLocalKeyWatcher(key, cb)
}

func (c *Client) AddLocalKeyWatcher(key string, cb Callback) error {
	needWatcher := false
	firstKey := false
	c.Lock()
